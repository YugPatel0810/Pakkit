...
import { Download, Save, CheckCircle, Clock, Truck, AlertTriangle } from 'lucide-react';
...
const Packages = () => {
  ...
  // Status icon renderer function
  const renderStatusIcon = (status) => {
    switch(status.toLowerCase()) {
      case 'delivered':
        return <div className="flex items-center"><CheckCircle className="h-4 w-4 text-green-500 mr-2" /> {status}</div>;
      case 'pending':
        return <div className="flex items-center"><Clock className="h-4 w-4 text-amber-500 mr-2" /> {status}</div>;
      case 'in transit':
        return <div className="flex items-center"><Truck className="h-4 w-4 text-blue-500 mr-2" /> {status}</div>;
      case 'delayed':
        return <div className="flex items-center"><AlertTriangle className="h-4 w-4 text-red-500 mr-2" /> {status}</div>;
      default:
        return status;
    }
  };
                ...
                <TableBody>
                  {packages.map(pkg => (
                    <TableRow key={pkg.id}>
                      ...
                      <TableCell>{renderStatusIcon(pkg.status)}</TableCell>
                      <TableCell>
          ...
          {selectedPackage && (
            <div className="grid gap-4 py-4">
              ...
              <div className="flex items-center">
                <strong>Status:</strong> <span className="ml-2">{renderStatusIcon(selectedPackage.status)}</span>
              </div>
              <div>
